...
// AI-Powered Categorization (Mocked for Hackathon)
export function classifyTransactionAI(merchant: string, amount: number): TransactionCategory {
  // ---------------------------------------------------------
  // STEP 1: ATTEMPT AI CLASSIFICATION (MOCKED LLM INTEGRATION)
  // ---------------------------------------------------------
  // In a production environment, this is where we would call an LLM API (like Gemini):
  //
  // const prompt = `
  //   You are an eco-financial categorizer. 
  //   Classify the following transaction into exactly one of these categories: 
  //   [fuel, electricity, flights, shopping, food, transport, other].
  //   
  //   Merchant Name: "${merchant}"
  //   Amount: ₹${amount}
  //   
  //   Respond ONLY with the category name in lowercase. Do not include any explanations.
  // `;
  // 
  // const response = await llm.generateText(prompt);
  // if (validCategories.includes(response.text.trim())) return response.text.trim();
  
  // For offline/hackathon purposes, we simulate the LLM's intelligence with advanced heuristics
  const merchantLower = merchant.toLowerCase();
  
  // Advanced Mock Intelligence: Context-aware parsing
  if (amount > 8000 && (merchantLower.includes('make') || merchantLower.includes('yatra') || merchantLower.includes('cleartrip'))) {
    return 'flights'; // High-value travel portal bookings are likely flights
  }
  
  if (merchantLower.includes('cafe') || merchantLower.includes('coffee') || merchantLower.includes('bakers')) {
    return 'food'; // Compound keyword detection
  }

  if (merchantLower.includes('blinkit') || merchantLower.includes('zepto') || merchantLower.includes('instamart')) {
    return 'food'; // Quick commerce
  }
  
  if (merchantLower.includes('decathlon') || merchantLower.includes('croma')) {
    return 'shopping'; // Specific large retailers
  }

  // ---------------------------------------------------------
  // STEP 2: FALLBACK TO KEYWORD LOGIC
  // ---------------------------------------------------------
  for (const [category, keywords] of Object.entries(CATEGORY_KEYWORDS)) {
    // Smarter matching: checks word boundaries or direct inclusions to avoid false positives
    if (keywords.some(keyword => merchantLower.includes(keyword) || keyword === merchantLower)) {
...
export function processTransactions(rawTransactions: Omit<Transaction, 'id' | 'category' | 'co2Emissions'>[]): Transaction[] {
  return rawTransactions.map((tx, index) => {
    const category = classifyTransactionAI(tx.merchant, tx.amount)
